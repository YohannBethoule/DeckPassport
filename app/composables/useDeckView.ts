...
function mergeColors(primary: string[], secondary?: string[]): ManaColor[] {
  const colorSet = new Set([...primary, ...(secondary ?? [])])
...
export function toDeckView(d: RawDeck): { deck: InsertDeckWithCommander, archetypeNames: string[] } {
  ...
  return {
    deck: {
      ...
      colors: mergeColors(d.commander.colors, d.partnerCommander?.colors),
      title: d.title,
