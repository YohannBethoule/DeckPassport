...
const { data: friends, refresh: refreshFriends } = await useFetch('/api/friends')
const { data: receivedRequests, refresh: refreshReceivedRequests } = await useFetch('/api/friends/requests', {
  query: { type: FRIEND_REQUEST_TYPE.RECEIVED }
})
const { data: sentRequests } = await useFetch('/api/friends/requests', {
...
<template>
  <UContainer>
    ...
    <UPageSection
      title="My Friends"
      :description="!friends.length && 'No friend already. Go on their profile page to send them a friend request.'"
    >
      <UPageList divide>
        <div
    ...
    <UPageSection
      ...
      :description="(!receivedRequests.length && !sentRequests.length) && 'No pending friend requests.'"
    >
      <UPageList divide>
        <div
